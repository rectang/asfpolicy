...
The canonical channel for distribution of current Apache software releases to
the general public is the Apache mirror network.
...
created is governed by the Apache Release Policy.
...
Such packages are sometimes referred to as "convenience binaries" to
...
Content suitable for Apache's canonical distribution channel includes:

*   Official releases
*   "Convenience binaries"
*   Cryptographic signatures and checksums
*   A `KEYS` file containing PGP keys
*   `README`, `CHANGES` and similar documents describing distributed
    content

If an Apache PMC wishes to distribute additional content through the mirror
network and there is any question about its suitability, the PMC MUST consult
with the Board.
...
SHA256 and SHA512 use the same SHA algorithm family with longer hash lengths (256 and 512 bits respectively). These longer variations are less vulnerable to the weaknesses found in the algorithm family than SHA1. SHA512 is recommended.
...
## Release Distribution Policy Administration ## {#administration}
